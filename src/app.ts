            ...
            if (permissions.isSuperUser) {
                if (packet.action && packet.action === 'takeOff') {
                    ...
                    if (canTakeOff() || packet.force === true) {
                        (async () => {
                            try {
                                const ftp = new FTP(ip);
                                const connected = await ftp.connect();
                                if (connected) {
                                    const localPath = join(paths[Paths.FLIGHT_PLANS], `${name}.mavlink`);
                                    const remotePath = `/internal_000/flightplans/${name}.mavlink`;
                                    
                                    logger.info(`Uploading flight plan to drone: ${remotePath}`);
                                    await ftp.upload(localPath, remotePath);
                                    ftp.disconnect();

                                    disco.Mavlink.start(name + '.mavlink');
                                    logger.info(`User start flight plan: ${name}`);
                                } else {
                                    throw new Error('Could not connect to drone FTP');
                                }
                            } catch (err) {
                                logger.error(`Flight plan start failed: ${err.message}`);
                                peer.send(JSON.stringify({
                                    action: 'alert',
                                    data: { level: 'danger', message: 'FTP Upload Failed' }
                                }));
                            }
                        })();
